...
An offline ingestion dry run now operationalizes these boundaries with a fixture-only source registry, mock adapters, retrieval provenance, idempotent ledger, deterministic retry and circuit breaking, quarantine and review queues, edition slots, reconciliation, and resume checkpoints. It is architecture rehearsal—not a publisher connection, production scheduler, Cloudflare binding, database, or deployment.

The next implemented boundary is `gold-corpus-v1`: 345 offline annotation candidates, blinded independent review contracts, adjudication, appropriate agreement metrics, leakage-safe partitions, threshold proposals, held-out replay, and regression promotion. Human decisions are not yet present, so the framework blocks calibration and live-pilot approval honestly. Future production review persistence and authentication remain planned.
